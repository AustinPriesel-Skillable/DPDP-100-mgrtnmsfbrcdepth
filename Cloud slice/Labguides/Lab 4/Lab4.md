...
In modern data engineering solutions, organizations are increasingly adopting unified analytics platforms like Microsoft Fabric to streamline data integration and processing. This lab provides hands-on experience in migrating existing Azure Data Factory (ADF) pipelines to Microsoft Fabric Data Factory. You will explore how to deploy an ADF instance, run and monitor pipelines, and then seamlessly migrate those pipelines into a Fabric workspace. By the end of this lab, you will understand the migration workflow, validate pipeline functionality in Fabric, and leverage OneLake for data storage and processing.
...
1. Open a browser go to +++portal.azure.com+++ sign in with your cloud slice account below.

    Username: +++@lab.CloudPortalCredential(User1).Username+++

    TAP: +++@lab.CloudPortalCredential(User1).AccessToken+++

    ![A screenshot of a computer Description automatically generated](https://raw.githubusercontent.com/technofocus-pte/mgrtnmsfbrcdepth/refs/heads/main/Cloud%20slice/Labguides/Lab%204/media/image1.png)

    ![A screenshot of a login box Description automatically generated](https://raw.githubusercontent.com/technofocus-pte/mgrtnmsfbrcdepth/refs/heads/main/Cloud%20slice/Labguides/Lab%204/media/image2.png)

1. Open a browser go to +++https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fquickstarts%2Fmicrosoft.datafactory%2Fdata-factory-get-started%2Fazuredeploy.json+++
...
1. After the deployment is completed, click on **Go to resource group** button.
...
1. Click on **Launch studio** to launch your Azure Data factory.
...
1. In the source dataset, select **Open** to view the input file created for the demo.
...
1. In the source dataset, select **Browse** to view the input file created
...
1. Once the Copy Data activity is configured, click **Add trigger** at the top to set up a trigger for running the pipeline
...
1. Select the **Monitor** tab  
    ...
    This tab provides an overview of your pipeline runs, including the start time and status.
...
1. Then, In the **Microsoft** window enter the password and click on the **Sign in** button.
    ...
    ![A login screen with a red box and blue text AI-generated content may be incorrect.](https://raw.githubusercontent.com/technofocus-pte/mgrtnmsfbrcdepth/refs/heads/main/Cloud%20slice/Labguides/Lab%204/media/image31.png)

1. In **Stay signed in?** window, click on the **Yes** button.

    ![A screenshot of a computer error AI-generated content may be incorrect.](https://raw.githubusercontent.com/technofocus-pte/mgrtnmsfbrcdepth/refs/heads/main/Cloud%20slice/Labguides/Lab%204/media/image32.png)
...
1. In the **Azure Data Factory-Fabric@lab.LabInstance.Id** workspace page, navigate and click on **+New item**  button
...
1. In the **New lakehouse** dialog box, enter +++DataFactory_MigrationLakehouse+++ in the +++Name+++ field, click on the **Create** button and open the new lakehouse.
...
1. From the **Manage** section, navigate to **Linked services** and then click on **Migrate to Fabric (Preview)** to begin the migration process
...
1. Review the migration assessment summary, and once the pipeline appears with a Ready status, click **Next** to proceed with the migration
...
1. Select the Fabric workspace name from the dropdown menu and click *Mount* to attach the Azure Data Factory to the chosen Fabric workspace
...
1. Once the mounting process completes successfully, click **Continue in Fabric** to proceed with the migration steps in the Fabric workspace.
...
1. Verify that the pipeline is marked as Ready in the migration summary, then click **Review connections** to continue with the migration process
...
1. Select the folder where you want the migrated items to be stored, and then click **Confirm** to proceed with the migration
...
1. After the migration completes and the status shows *Success*, click **Close** to exit the migration results window.
...
1. Navigate to **Source** tab. Select the **Connection** dropdown and select **Browse all** option.
...
1. On choose a destination window, select **OneLake catalog** from the left pane and select the **DataFactory_MigrationLakehouse**.
...
1. In the Destination settings, select the **Files** option and set the file format to **Binary** before configuring the remaining parameters.
...
1. After the successful execution of the pipeline, go to your Lakehouse
...
1. Now, click on **workspace** on the left-sided navigation pane and select **pipeline**
...
1. Select **General** and **Remove this workspace.**
    ...
    ![A screenshot of a computer AI-generated content may be incorrect.](https://raw.githubusercontent.com/technofocus-pte/mgrtnmsfbrcdepth/refs/heads/main/Cloud%20slice/Labguides/Lab%204/media/image79.png)
...
In this lab, you successfully deployed an Azure Data Factory instance and executed a sample data pipeline. You then created a Microsoft Fabric workspace and Lakehouse to serve as the destination environment. Using the built-in migration feature, you transferred the ADF pipeline to Fabric Data Factory, validated its configuration, and executed it to ensure successful data ingestion into OneLake. Finally, you explored scheduling capabilities and learned how to manage and clean up resources. This lab demonstrates how organizations can modernize their data integration workflows by transitioning from Azure Data Factory to Microsoft Fabric.
